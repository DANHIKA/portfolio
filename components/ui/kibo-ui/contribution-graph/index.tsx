...
export const ContributionGraph = ({
  ...
  useEffect(() => {
    const cutoff = subMonths(new Date(), 4);
    const recentData = data.filter((a) => parseISO(a.date) >= cutoff);
    const randomized = recentData.map((activity) => {
      if (activity.level === 0 && Math.random() > 0.2) {
        return { ...activity, level: Math.floor(Math.random() * 4) + 1 };
      }
      return activity;
    });
    setRandomizedData(randomized);
  }, [data]);
  const filteredData = randomizedData;

  const weeks = useMemo(() => groupByWeeks(filteredData, weekStart), [filteredData, weekStart]);
  ...
  const year =
    filteredData.length > 0
      ? getYear(parseISO(filteredData[0].date))
      : new Date().getFullYear();

  const totalCount =
    typeof totalCountProp === "number"
      ? totalCountProp
      : filteredData.reduce((sum, activity) => sum + activity.count, 0);
