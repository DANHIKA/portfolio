...
type DrawTextProps = {
  ...
  textColor?: "primary" | "white" | "foreground" | string;
  as?: HeadingTag;
  className?: string;
};
...
const animatePath = (
  ...
  path.style.strokeDashoffset = `${totalLength * (1 - fromStart / 100)}`;
  ...
  const animate = (currentTime: number) => {
    ...
    const currentOffset =
      totalLength * (1 - (fromStart + (toEnd - fromStart) * eased) / 100);
...
export const DrawText: React.FC<DrawTextProps> = ({
  ...
  const currentIndex = useRef(Math.floor(Math.random() * svgVariants.length));
  ...
  const triggerAnimation = useCallback(() => {
    ...
    boxRef.current.innerHTML = svgVariants[currentIndex.current];
    const path = boxRef.current.querySelector("path");

    if (path) {
      animatePath(path, 0, 100, 0.8, () => {
    ...
    currentIndex.current = (currentIndex.current + 1) % svgVariants.length;
  }, []);
  ...
  }, [triggerAnimation, text, children]);
  ...
  return (
    ...
    >
      <span className="draw-text__text">{children ?? text}</span>
      <span ref={boxRef} className="draw-text__box" aria-hidden />
      ...
      <style jsx>{`
        ...
        .draw-text__box :global(path) {
          stroke: hsl(var(--primary)) !important;
        }

        .draw-text--white .draw-text__text {
          color: white;
        }
      `}</style>
    </Component>
